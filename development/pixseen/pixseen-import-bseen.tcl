...
# pixseen-import-bseen.tcl --
#
#       This shell script imports a bseen database to pixseen.tcl.
# 
...
if {$argv eq {}} {
	puts "Usage: $argv0 <path/to/bs_data_file>"
	exit
}
if {![file exists $argv]} {
...
if {[info exists ::version]} {
	putlog "ERROR! Detected pixseen-import-bseen.tcl being loaded from Eggdrop. This script is meant to be run as a shell script."
	die "PEBKAC detected"
}
...
namespace eval ::pixseen {
	...
	set bsVer [gets $fd]
	if {$bsVer ne {#10402}} {
		puts stdout "The bseen database has to be version 1.4.2. Please update your bseen database and try again."
		exit
	}
	...
	while {![eof $fd]} {
		...
		lassign $line nick uhost timestamp event chan
		set msg [join [lrange $line 5 end]]
		...
		switch -exact -- $event {
			...
			{kick} {
				set aggressor [lindex $line 5]
				set msg [lrange $line 6 end]
			...
			{away} {
				if {$msg eq {}} {
					...
					# away
					set botname $chan
					if {[catch {seendb eval { INSERT OR REPLACE INTO seenTb VALUES(12, $nick, $uhost, $timestamp, NULL, $msg, $botname) }} error]} {
						puts stdout "SQL error: [seendb errorcode] $error"
						exit
					}
				}
